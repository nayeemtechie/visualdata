 ...
 * Chart displaying Attributable Sales over time with multi-metric overlay
 */
const SalesVelocityChart = ({ data = [], columnMapping = {} }) => {
    const [granularity, setGranularity] = useState(GRANULARITY.DAILY);
    const [hiddenMetrics, setHiddenMetrics] = useState(new Set());

    // Build available metrics from column mapping
    const availableMetrics = useMemo(() => {
        const metrics = [];

        // Primary metric: Attributable Sales
        if (columnMapping.attributableSales?.column) {
            metrics.push({
                field: 'attributableSales',
                label: columnMapping.attributableSales.label || 'Attributable Sales',
                aggregationType: AGGREGATION_TYPE.SUM,
                color: columnMapping.attributableSales.color || CHART_COLORS[0],
                isPercentage: columnMapping.attributableSales.isPercentage || false,
                formula: columnMapping.attributableSales.formula,
            });
        }

        // Add other mapped metrics
        Object.entries(columnMapping).forEach(([field, config]) => {
            if (field === 'date' || field === 'attributableSales' || field === 'ctr') return;
    ...
    // Build column mapping for aggregator (field -> actual column name)
    ...
    const chartData = useMemo(() => {
        ...
        return baseChartData.map(row => {
            ...
            formulaMetrics.forEach(metric => {
                // For formula metrics, we need to aggregate the formula results
    ...
    const metricTotal = primaryMetric
        ? getMetricTotal(primaryMetric.field, AGGREGATION_TYPE.SUM)
        : undefined;
    ...
    if (isEmpty || availableMetrics.length === 0) {
        return (
            <ChartCard
                title="Attributable Sales Velocity"
                subtitle="Attributable Sales over time"
            ...
            >
                <div className="h-full flex items-center justify-center text-gray-400">
                    <div className="text-center">
                        ...
                        <p className="text-sm mt-1">Map "Attributable Sales" column to see data</p>
                    </div>
                </div>
            </ChartCard>
        );
                ...
                <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                    <defs>
                        {validSelectedMetrics.map(metric => (
                            <linearGradient key={metric.field} id={`gradient-${metric.field}`} x1="0" y1="0" x2="0" y2="1">
                    ...
                    {validSelectedMetrics.map(metric => (
                        <Area
                            ...
                            stroke={metric.color}
                            strokeWidth={2}
                            fill={`url(#gradient-${metric.field})`}
